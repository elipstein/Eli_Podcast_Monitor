...
def to_html(flagged: list[FlaggedEpisode], generated_at: datetime | None = None, note: str = "") -> str:
    ...
    note_html = f'<p class="subtitle" style="margin-bottom:1.5rem;">{escape(note)}</p>' if note else ""

    if not flagged:
        body = '<p class="empty">No episodes matched the criteria in the most recent scan.</p>'
        return _HTML_TEMPLATE.format(generated_at=generated_str, body=body, note=note_html)
